...
__k_handle_reply()
{
    ...
    if [[ ${#COMPREPLY[@]} -eq 0 ]]; then
		if declare -F __k_custom_func >/dev/null; then
...
__k_handle_flag()
{
    ...
    if [[ ${words[c]} != *"="* ]] && __k_contains_word "${words[c]}" "${two_word_flags[@]}"; then
			  __k_debug "${FUNCNAME[0]}: found a flag ${words[c]}, skip the next argument"
        c=$((c+1))
...
__k_parse_config()
{
    ...
    if k_out=$(__k_debug_out "k config $(__k_override_flags) -o template --template=\"${template}\" view"); then
...
__k_parse_get()
{
    ...
    local k_out
    if k_out=$(__k_debug_out "k get $(__k_override_flags) -o template --template=\"${template}\" \"$1\""); then
        COMPREPLY+=( $( compgen -W "${k_out[*]}" -- "$cur" ) )
    fi
}
...
__k_get_resource()
{
    if [[ ${#nouns[@]} -eq 0 ]]; then
      local k_out
      if k_out=$(__k_debug_out "k api-resources $(__k_override_flags) -o name --cached --request-timeout=5s --verbs=get"); then
          COMPREPLY=( $( compgen -W "${k_out[*]}" -- "$cur" ) )
          return 0
      fi
      return 1
...
__k_get_containers()
{
    ...
    local k_out
    if k_out=$(__k_debug_out "k get $(__k_override_flags) -o template --template=\"${template}\" pods \"${last}\""); then
        COMPREPLY=( $( compgen -W "${k_out[*]}" -- "$cur" ) )
    fi
}
...
__k_cp()
{
    ...
    case "$cur" in
        ...
        */*) # complete <namespace>/<pod>
            ...
            namespace="${cur%%/*}"
            if k_out=$(__k_debug_out "k get $(__k_override_flags) --namespace \"${namespace}\" -o template --template=\"${template}\" pods"); then
                COMPREPLY=( $(compgen -W "${k_out[*]}" -- "${cur}") )
            fi
            return
            ;;
...
__k_bash_source <(__k_convert_bash_to_zsh)
_complete k 2>/dev/null
